...
Either install path resolves a platform-matched native runtime (`kandev`, agentctl, Next.js standalone web), launches the backend + web, and opens your browser. Data (worktrees, SQLite DB) is stored in `~/.kandev` by default.
...
- Node.js (for `npx`; Homebrew installs run the native `kandev` binary directly)
- Git
