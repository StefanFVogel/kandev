...
flowchart TD
    ...
    D2 --> E["exec bin/kandev"]
    D4 --> E
    H --> E2["Start kandev __backend + Next.js standalone"]
    E --> E2
```
...
Both channels resolve to the same runtime bundle layout (`bin/kandev`, `bin/agentctl`, `web/server.js`). The public command remains `kandev`; the hidden backend mode is `kandev __backend`.
